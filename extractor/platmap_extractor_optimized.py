...
Architecture:
  Step 1: PageFinder (YOLO + OCR)  — Find correct PDF page using plat_book +
                                      page from input JSON. YOLO detects
                                      title_block region on each thumbnail,
                                      OCR reads plat book/page from that crop.
                                      Falls back to corner OCR if YOLO not trained.

  Step 2: YOLO (trained model)     — Detect all layout regions on thumbnail:
                                      lot, curve_table, line_table,
                                      title_block, legend.
                                     Falls back to Groq if YOLO not trained.

  Step 3: PyMuPDF                  — Crop ALL detected regions from ORIGINAL
...
Dependencies (same directory):
  page_finder.py     — PageFinder class (YOLO + OCR page detection)
  region_detector.py — YOLODetector class (layout detection)

YOLO model path: set YOLO_MODEL_PATH in page_finder.py and below.
Until trained: set to None → fallback modes activate automatically.
"""
...
# page_finder.py   : PageFinder — YOLO detects title_block → OCR matches plat_book/page
# region_detector.py: YOLODetector — detects lot, tables, legend, title_block
from page_finder import PageFinder, SearchInput
from region_detector import YOLODetector
...
class PlatMapExtractor:
    ...
    Step 1 — PageFinder (YOLO + OCR):
      Uses your trained YOLO model to detect the title_block region on
      each page thumbnail, then OCRs that crop to match plat_book + page.
      Falls back to corner OCR when YOLO_MODEL_PATH = None.

    Step 2 — YOLODetector (YOLO):
      Detects all layout regions (lot, curve_table, line_table,
      title_block, legend) on a thumbnail of the correct page.
      Falls back to Groq dynamic detection when YOLO not trained.

    Step 3-5 — unchanged from original.
    """

    def __init__(self, groq_api_key: str, claude_api_key: str):
        self.groq_client   = Groq(api_key=groq_api_key)
        self.claude_client = anthropic.Anthropic(api_key=claude_api_key)
        # YOLO-based page finder (page_finder.py)
        self.page_finder   = PageFinder()
        # YOLO-based layout detector (region_detector.py)
        self.yolo_detector = YOLODetector()
        print(f"[Extractor] Initialized")
        yolo_page = self.page_finder._yolo_available()
        print(f"[Extractor]   Page detection  : PageFinder "
              f"({'YOLO+OCR' if yolo_page else 'OCR-fallback'})")
        print(f"[Extractor]   Layout detection: YOLODetector "
              f"({'YOLO' if self.yolo_detector.available else 'Groq-fallback'})")

    def visualize_detections(self, img: Image.Image,
                              detected_regions: Dict,
                              output_path: str):
        """Create visualization of all detected regions."""
        ...
        for i, t in enumerate(detected_regions.get('all_tables', [])):
            b = t.get('bbox') or t.get('bbox_pct')
            if b:
                bboxes.append(b)
                labels.append(f"Table {i+1}: {t.get('type', t.get('class','?'))}")
                colors.append('orange')

        legend = detected_regions.get('legend', {})
        if legend.get('found') and (legend.get('bbox') or legend.get('bbox_pct')):
            bboxes.append(legend.get('bbox') or legend.get('bbox_pct'))
            labels.append("Legend")
            colors.append('blue')

        title = detected_regions.get('title_block', {})
        if title.get('found') and (title.get('bbox') or title.get('bbox_pct')):
            bboxes.append(title.get('bbox') or title.get('bbox_pct'))
    ...
    def extract(self, pdf_path: str, lot_number: str,
        ...
        Args:
            ...
            lot_number:     Lot number to extract (e.g. "112")
            block_number:   Optional block number
            page_number:    Override page index — skips PageFinder if set
            plat_book:      Plat book number from input JSON (e.g. "16")
            plat_page:      Plat page number from input JSON (e.g. "198")
            visualize_path: Optional path to save detection visualization PNG
        ...
        if page_number is not None:
        ...
        elif plat_book and plat_page:
            print(f"[Step 1] PageFinder: Book={plat_book} Page={plat_page}...")
            search      = SearchInput(
                plat_book = plat_book,
                page      = plat_page,
                lot       = lot_number,
            ...
            found       = self.page_finder.find_page(pdf_path, search)
            page_number = found if found is not None else 0
            if found is None:
                print(f"[Step 1] ⚠️  No match — defaulting to page 0")
        else:
            print(f"[Step 1] ⚠️  No plat_book/page provided — using page 0")
            page_number = 0
        ...
        if self.yolo_detector.available:
            # YOLO path — fast, free, local
            detected_regions = self._detect_with_yolo(
                full_image, lot_number, block_number
            )
        else:
            # Groq fallback — until YOLO model is trained
            print(f"[Step 2] YOLO not trained — using Groq fallback...")
        ...
        result["detection_summary"]  = detected_regions.get(
            "detection_summary", {}
        )
    ...
    def _detect_with_yolo(self, full_image: Image.Image,
                           lot_number: str,
                           block_number: Optional[str]) -> Dict:
        """
        Uses YOLODetector to find all regions on thumbnail,
        then returns them in the same dict structure that
        _create_crops() and visualize_detections() expect.
        """
        orig_size = full_image.size
        ow, oh    = orig_size

        # Make thumbnail for YOLO (ORIGINAL never resized)
        from region_detector import _make_thumbnail
        thumbnail, _, _ = _make_thumbnail(full_image, 1200)

        detections = self.yolo_detector.detect(thumbnail, orig_size)

        # Separate by class
        lot_dets   = [d for d in detections if d["class"] == "lot"]
        table_dets = [d for d in detections
                      if d["class"] in ("curve_table", "line_table")]
        legend_det = next((d for d in detections
                           if d["class"] == "legend"), None)
        title_det  = next((d for d in detections
                           if d["class"] == "title_block"), None)

        print(f"[YOLO] Lots={len(lot_dets)} Tables={len(table_dets)} "
              f"Legend={1 if legend_det else 0} "
              f"Title={1 if title_det else 0}")

        # ── Find specific lot via OCR + Claude micro-confirm ──
        lot_bbox_orig = None
        if lot_dets:
            from region_detector import (LotFinder,
                                          _make_thumbnail as _mt)
            finder        = LotFinder(self.claude_client)
            lot_bbox_orig = finder.find_lot(
                full_image, lot_number, lot_dets
            )

        # ── Build detected_regions dict ──
        # Use bbox_pct so _create_crops / visualize work with both paths
        def to_pct(bbox_orig):
            x0,y0,x1,y1 = bbox_orig
            return {"x0":x0/ow,"y0":y0/oh,"x1":x1/ow,"y1":y1/oh}

        lot_region = {"found": False}
        if lot_bbox_orig:
            lot_region = {
                "found": True,
                "bbox":  to_pct(lot_bbox_orig),
                "bbox_orig": lot_bbox_orig,
            }
        ...
        detected_regions = {
            ...
            "legend": {
                "found": True,
                "bbox":  legend_det["bbox_pct"],
                "bbox_orig": legend_det["bbox_orig"],
            } if legend_det else {"found": False},
            "title_block": {
                "found": True,
                "bbox":  title_det["bbox_pct"],
                "bbox_orig": title_det["bbox_orig"],
            } if title_det else {"found": False},
            "detection_summary": {
                "lot_found":       lot_region["found"],
                "tables_count":    len(all_tables),
                "legend_found":    bool(legend_det),
                "title_found":     bool(title_det),
                "detection_mode":  "YOLO",
            }
        }
    ...
    def _create_crops(self, img: Image.Image,
                       detected_regions: Dict) -> Dict[str, Image.Image]:
        """Create crops from all detected regions."""
        crops = {}
        ow, oh = img.size

        # Lot crop — prefer bbox_orig (pixel-accurate) over bbox_pct
        lot_region = detected_regions.get("lot", {})
        if lot_region.get("found"):
            if lot_region.get("bbox_orig"):
                x0,y0,x1,y1 = lot_region["bbox_orig"]
                crops["lot"] = crop_lot_region(
                    img, lot_region["bbox_orig"]
                )
            elif lot_region.get("bbox"):
                crops["lot"] = crop_lot_region(
                    img, lot_region["bbox"]
                )
        ...
        # Table crops — prefer bbox_orig for pixel-accurate YOLO crops
        for idx, table in enumerate(detected_regions.get("all_tables", [])):
            table_type = table.get("type", table.get("class", "unknown"))
            if table.get("bbox_orig"):
                x0,y0,x1,y1 = table["bbox_orig"]
                pad = 25
                x0 = max(0,  x0 - pad); y0 = max(0,  y0 - pad)
                x1 = min(ow, x1 + pad); y1 = min(oh, y1 + pad)
                cropped = img.crop((x0,y0,x1,y1))
                if cropped.width > TABLE_MAX_WIDTH:
                    r = TABLE_MAX_WIDTH / cropped.width
                    cropped = cropped.resize(
                        (TABLE_MAX_WIDTH, int(cropped.height*r)),
                        Image.LANCZOS
                    )
                crops[f"{table_type}_{idx+1}"] = cropped
            elif table.get("bbox"):
                crops[f"{table_type}_{idx+1}"] = crop_table_region(
                    img, table["bbox"]
                )
            print(f"[Cropper] ✅ {table_type}_{idx+1}: "
                  f"{crops[f'{table_type}_{idx+1}'].size}")

        # Legend
        legend = detected_regions.get("legend", {})
        if legend.get("found"):
            bbox = legend.get("bbox_orig")
            if bbox:
                crops["legend"] = img.crop(bbox)
            elif legend.get("bbox"):
        ...
        title = detected_regions.get("title_block", {})
        if title.get("found"):
            bbox = title.get("bbox_orig")
            if bbox:
                crops["title_block"] = img.crop(bbox)
            elif title.get("bbox"):
                crops["title_block"] = crop_region(img, title["bbox"])
            if "title_block" in crops:
                print(f"[Cropper] ✅ Title block: "
                      f"{crops['title_block'].size}")
...
if __name__ == "__main__":
    ...
    # Input JSON — user provides plat_book + page + lot + block
    result = extractor.extract(
        pdf_path       = "plats/plat8.pdf",
        lot_number     = "112",
        block_number   = None,
        plat_book      = "16",    # from input JSON
        plat_page      = "198",   # from input JSON
